...
void runSearch(vector<Point> *points, double *minimumPointStartingRange, double *maximumPointStartingRange, chrono::time_point<chrono::high_resolution_clock> start) {
    ...
    // cout << "second red count: " << redsInRange << endl;
...
string getCSVField(stringstream &ss, char delim) {
    string field;
    if (ss.peek() == '"') {
        ss.get(); // consume the opening quote
        getline(ss, field, '"'); // read until closing quote
        // After a quoted field, consume the delimiter if present.
        if (ss.peek() == delim)
            ss.get();
    } else {
...
vector<Point> *readPoints(string filename, double *minimum, double *maximum, int &redInRange, int &blueInRange) {
    ...
    while (!file.eof()) {
        ...
        Point point;
        point.coordinates = new double[2];
        string age1 = getCSVField(ss, ',');
        string age2 = getCSVField(ss, ',');
        string b = getCSVField(ss, ',');
        int age1_int = std::stoi(age1); 
        int age2_int = std::stoi(age2); 
        int b_int = std::stoi(b);
        point.coordinates[0] = age1_int;
        point.coordinates[1] = age2_int;
        // ss >> point.coordinates[0];
        // ss >> point.coordinates[1];
        int isBlue;
        //1 is blue?
...
//     string filename = "/Users/naser-ir/Desktop/compasdataset/sorted.csv";
...
int main(int argc, char* argv[]) {
    if (argc < 5) {
        cout << "Usage: ./2d_unweighted_pip_final min_x min_y max_x max_y" << endl;
        return 1;
    }
    ...
    minimum[0] = atof(argv[1]);
    minimum[1] = atof(argv[2]);
    ...
    std::ofstream outputFile("output.txt"); // Create and open a file

    if (outputFile.is_open()) { // Check if the file is open
        outputFile << minimum[0];
        outputFile << "\n";
        outputFile << maximum[0];
        outputFile << "\n";
        outputFile << frange[0];
        outputFile << "\n";
        outputFile << frange[1];
